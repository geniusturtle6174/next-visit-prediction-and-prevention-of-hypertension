...
parser.add_argument('--model_type', default='xgb', choices=('xgb', 'lgbm'))
...
for fold in range(args.n_fold):
    ...
    # 1st pass
    models[fold] = Classifier(**tr_param['param']) if tr_param['run_classification_prob'] else XGBRegressor(**tr_param['param'])
    models[fold].fit(
        fea_all[train_idx],
        ans_all[train_idx],
        eval_set=[
            (fea_all[valid_idx], ans_all[valid_idx]),
            (fea_all[train_idx], ans_all[train_idx]),
        ],
    )

    # 2nd pass
    if tr_param['model_type'] == 'xgb':
        va_loss = models[fold].evals_result()['validation_0']['logloss' if tr_param['run_classification_prob'] else 'rmse']
        tr_loss = models[fold].evals_result()['validation_1']['logloss' if tr_param['run_classification_prob'] else 'rmse']
    elif tr_param['model_type'] == 'lgbm':
        va_loss = models[fold].evals_result_['valid_0']['binary_logloss' if tr_param['run_classification_prob'] else 'rmse']
        tr_loss = models[fold].evals_result_['valid_1']['binary_logloss' if tr_param['run_classification_prob'] else 'rmse']
    local_param = deepcopy(tr_param['param'])
    local_param['n_estimators'] = np.argsort(va_loss)[0] + 1
    models[fold] = Classifier(**local_param) if tr_param['run_classification_prob'] else XGBRegressor(**local_param)
    models[fold].fit(fea_all[train_idx], ans_all[train_idx])
    best_va_loss_all.append(np.min(va_loss))
    best_n_est_all.append(local_param['n_estimators'])

    # Predict
    va_hybp_prob = models[fold].predict_proba(fea_all[valid_idx])[:, 1]
    
    # Save
    util.save_pkl(os.path.join(args.save_model_dir_name, 'model_{}.pkl'.format(fold)), models[fold])
    np.save(os.path.join(args.save_model_dir_name, 'va_loss_{}.npy'.format(fold)), va_loss)
    np.save(os.path.join(args.save_model_dir_name, 'tr_loss_{}.npy'.format(fold)), tr_loss)
